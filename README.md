...
指定プレイヤー(省略時は自分)のデバッグモードを切り替えます。有効な間は以下の要件チェックを全て無視して武器・スキルを使用できます。

- 武器: 職業要件、レベル要件
- スキル: 装備中武器の種類一致、ソケット済みか、習得済みか、クールダウン、SPコスト

武器レベルアップの上限やスキル習得ポイントのコストなど「育成」に関わる制限は対象外です(バイパス対象は「使用できるかどうか」のみ)。インメモリのフラグなので再ログインすると無効に戻ります。
